...
const apiBaseUrl = "https://concurso-publico-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Concurso</li>
          <li>✓ Controle completo de Candidato</li>
          <li>✓ Controle completo de Inscricao</li>
          <li>✓ Controle completo de Resultado</li>
        </ul>
      </div>
...
function PainelConcurso({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["titulo", "orgao", "vagas", "dataAbertura", "dataEncerramento", "edital", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/concursos", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/concursos/" + editId : apiBaseUrl + "/api/concursos";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["titulo", "orgao", "vagas", "dataAbertura", "dataEncerramento", "edital", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/concursos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["titulo"] ?? "Concurso")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/concursos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["titulo"] ?? "Concurso")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>orgao</b><span>{item["orgao"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>dataEncerramento</b><span>{item["dataEncerramento"] ? new Date(item["dataEncerramento"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>edital</b><span>{item["edital"] ?? "—"}</span></div>
      ...
      <label className="field-label">dataEncerramento</label>
      <input type="datetime-local" value={form["dataEncerramento"] ?? ""} onChange={e => setForm({...form, dataEncerramento: e.target.value})} />
      <label className="field-label">edital</label>
      <input type="text" value={form["edital"] ?? ""} onChange={e => setForm({...form, edital: e.target.value})} />
...
function PainelCandidato({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "dataNascimento", "escolaridade", "pcd", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/candidatos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/candidatos/" + editId : apiBaseUrl + "/api/candidatos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "dataNascimento", "escolaridade", "pcd"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/candidatos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Candidato")}</div>
            ...
            <div className="item-field"><b>escolaridade</b><span>{item["escolaridade"] ?? "—"}</span></div>
            <div className="item-field"><b>pcd</b><span>{item["pcd"] != null ? (item["pcd"] ? "Sim" : "Não") : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Candidato</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <input type="text" value={form["escolaridade"] ?? ""} onChange={e => setForm({...form, escolaridade: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["pcd"] === true || form["pcd"] === "true"} onChange={e => setForm({...form, pcd: e.target.checked})} />
        pcd
      </label>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
...
function PainelInscricao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [concursoList, setConcursoList] = React.useState([]);
  const [candidatoList, setCandidatoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["candidatoId", "concursoId", "cargo", "dataInscricao", "taxaPaga", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/inscricaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/concursos", { headers: authHeaders(token) }).then(r => r.json()).then(setConcursoList).catch(() => {});
    fetch(apiBaseUrl + "/api/candidatos", { headers: authHeaders(token) }).then(r => r.json()).then(setCandidatoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/inscricaos/" + editId : apiBaseUrl + "/api/inscricaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["candidatoId", "concursoId", "cargo", "dataInscricao", "taxaPaga", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["candidato", "concurso"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/inscricaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["cargo"] ?? "Inscricao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/inscricaos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["cargo"] ?? "Inscricao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>candidatoId</b><span>{item["candidatoId"] ?? "—"}</span></div>
            <div className="item-field"><b>concursoId</b><span>{item["concursoId"] ?? "—"}</span></div>
            <div className="item-field"><b>dataInscricao</b><span>{item["dataInscricao"] ? new Date(item["dataInscricao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>taxaPaga</b><span>{item["taxaPaga"] != null ? (item["taxaPaga"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>candidato</b><span>{item.candidatoId ? ((candidatoList.find(o => o.id === item.candidatoId) || {}).nome ?? ("#" + item.candidatoId)) : "—"}</span></div>
            <div className="item-field"><b>concurso</b><span>{item.concursoId ? ((concursoList.find(o => o.id === item.concursoId) || {}).titulo ?? ("#" + item.concursoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Inscricao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">candidatoId</label>
      <input type="number" value={form["candidatoId"] ?? ""} onChange={e => setForm({...form, candidatoId: e.target.value})} />
      <label className="field-label">concursoId</label>
      <input type="number" value={form["concursoId"] ?? ""} onChange={e => setForm({...form, concursoId: e.target.value})} />
      <label className="field-label">cargo</label>
      <input type="text" value={form["cargo"] ?? ""} onChange={e => setForm({...form, cargo: e.target.value})} />
      <label className="field-label">dataInscricao</label>
      <input type="datetime-local" value={form["dataInscricao"] ?? ""} onChange={e => setForm({...form, dataInscricao: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["taxaPaga"] === true || form["taxaPaga"] === "true"} onChange={e => setForm({...form, taxaPaga: e.target.checked})} />
        taxaPaga
      </label>
      <label className="field-label">status</label>
      <select value={form["status"] ?? "ATIVO"} onChange={e => setForm({...form, status: e.target.value})}>
      ...
      <label className="field-label">candidato</label>
      <select value={form["candidato"] ?? ""} onChange={e => setForm({...form, candidato: e.target.value})}>
        <option value="">Selecione...</option>
        {(candidatoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">concurso</label>
      <select value={form["concurso"] ?? ""} onChange={e => setForm({...form, concurso: e.target.value})}>
        <option value="">Selecione...</option>
        {(concursoList || []).map(o => (<option key={o.id} value={o.id}>{o.titulo ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelResultado({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [inscricaoList, setInscricaoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["inscricaoId", "notaObjetiva", "notaRedacao", "notaPratica", "classificacao", "aprovado", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/resultados", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/inscricaos", { headers: authHeaders(token) }).then(r => r.json()).then(setInscricaoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/resultados/" + editId : apiBaseUrl + "/api/resultados";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["inscricaoId", "notaObjetiva", "notaRedacao", "notaPratica", "classificacao", "aprovado"].forEach(k => { f[k] = item[k] ?? ""; });
    ["inscricao"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/resultados/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{"Resultado"}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>inscricaoId</b><span>{item["inscricaoId"] ?? "—"}</span></div>
            <div className="item-field"><b>notaObjetiva</b><span>{item["notaObjetiva"] != null ? Number(item["notaObjetiva"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>notaRedacao</b><span>{item["notaRedacao"] != null ? Number(item["notaRedacao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>notaPratica</b><span>{item["notaPratica"] != null ? Number(item["notaPratica"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>classificacao</b><span>{item["classificacao"] ?? "—"}</span></div>
            <div className="item-field"><b>aprovado</b><span>{item["aprovado"] != null ? (item["aprovado"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>inscricao</b><span>{item.inscricaoId ? ((inscricaoList.find(o => o.id === item.inscricaoId) || {}).cargo ?? ("#" + item.inscricaoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Resultado</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">inscricaoId</label>
      <input type="number" value={form["inscricaoId"] ?? ""} onChange={e => setForm({...form, inscricaoId: e.target.value})} />
      <label className="field-label">notaObjetiva</label>
      <input type="number" step="0.01" value={form["notaObjetiva"] ?? ""} onChange={e => setForm({...form, notaObjetiva: e.target.value})} />
      <label className="field-label">notaRedacao</label>
      <input type="number" step="0.01" value={form["notaRedacao"] ?? ""} onChange={e => setForm({...form, notaRedacao: e.target.value})} />
      <label className="field-label">notaPratica</label>
      <input type="number" step="0.01" value={form["notaPratica"] ?? ""} onChange={e => setForm({...form, notaPratica: e.target.value})} />
      <label className="field-label">classificacao</label>
      <input type="number" value={form["classificacao"] ?? ""} onChange={e => setForm({...form, classificacao: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["aprovado"] === true || form["aprovado"] === "true"} onChange={e => setForm({...form, aprovado: e.target.checked})} />
        aprovado
      </label>
      <label className="field-label">inscricao</label>
      <select value={form["inscricao"] ?? ""} onChange={e => setForm({...form, inscricao: e.target.value})}>
        <option value="">Selecione...</option>
        {(inscricaoList || []).map(o => (<option key={o.id} value={o.id}>{o.cargo ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Concurso");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Concurso Publico<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Concurso" ? " active" : "")} onClick={() => setAba("Concurso")}><span className="nav-ico">🗂️</span>Concurso</button>
        <button className={"nav-btn" + (aba === "Candidato" ? " active" : "")} onClick={() => setAba("Candidato")}><span className="nav-ico">🗂️</span>Candidato</button>
        <button className={"nav-btn" + (aba === "Inscricao" ? " active" : "")} onClick={() => setAba("Inscricao")}><span className="nav-ico">🗂️</span>Inscricao</button>
        <button className={"nav-btn" + (aba === "Resultado" ? " active" : "")} onClick={() => setAba("Resultado")}><span className="nav-ico">🗂️</span>Resultado</button>
      ...
      <main className="main">
        ...
        {aba === "Concurso" && <PainelConcurso token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Candidato" && <PainelCandidato token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Inscricao" && <PainelInscricao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Resultado" && <PainelResultado token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
